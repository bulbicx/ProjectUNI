...
import { searchBuyProperty, isThereNextPageBuy, isThereNextNextPageBuy } from '../../redux/ActionCreators'
...
function SearchBar(props) {
    ...
    const searchStuff = (search) => {
        dispatch(searchBuyProperty(search, scale(value), school, train, numBeds, crimeRate, props.page, limitPage))
    
        dispatch(isThereNextPageBuy(search, scale(value), school, train, numBeds, crimeRate, props.page + 1, limitPage))

        dispatch(isThereNextNextPageBuy(search, scale(value), school, train, numBeds, crimeRate, props.page + 2, limitPage))
    }
    ...
    useEffect(() => {//Whenever page changes make search
        dispatch(searchBuyProperty(props.criteria ? props.criteria : searchValue, scale(value), school, train, numBeds, crimeRate, props.page, limitPage))

        dispatch(isThereNextPageBuy(props.criteria ? props.criteria : searchValue, scale(value), school, train, numBeds, crimeRate, props.page + 1, limitPage))

        dispatch(isThereNextNextPageBuy(props.criteria ? props.criteria : searchValue, scale(value), school, train, numBeds, crimeRate, props.page + 2, limitPage))
    }, [props.page])
    ...
    const handleClickSearch = () => {
        ...
        props.setPage(1)
        dispatch(searchBuyProperty(searchValue, scale(value), school, train, numBeds, crimeRate, props.page, limitPage))
        dispatch(isThereNextPageBuy(searchValue, scale(value), school, train, numBeds, crimeRate, props.page + 1, limitPage))
        dispatch(isThereNextNextPageBuy(searchValue, scale(value), school, train, numBeds, crimeRate, props.page + 2, limitPage))
    }
                ...
                <div className={classes.sub_filter_container}>
                    <div style={{backgroundColor: moneyChecked && '#F9E79F'}} className={classes.price_box}>
                        ...
                        <FormControlLabel 
                            ...
                            label={<span style={{ fontSize: '1rem', fontWeight: "bold" }}>{value == 75 ? 'Any price' : `Up to £${scale(value)}`}</span>}
                            labelPlacement="left"
                        />
                        {moneyChecked &&
                    ...
                    <div style={{backgroundColor: schoolChecked && '#F9E79F'}} className={classes.school_section}>
                        <div style={{position: 'absolute', width: '100%'}}>
                        <FormControlLabel 
                            control={
                                <Checkbox 
                    ...
                    <div className={classes.crime_section} style={{backgroundColor: crimeChecked && '#F9E79F'}} >
                        ...
                        {crimeChecked &&
                            <div className={classes.crime_sub_box}>
                                ...
                                <div className={classes.crime_div} onClick={() => setCrimeRateInput(100)}>
                    ...
                    <div className={classes.btn_section}>
                        <button onMouseDown={e => e.preventDefault()} onClick={handleClickSearch} className={classes.btn}>Search</button>
                    </div>
                    </div>
                </div>
            </div>
