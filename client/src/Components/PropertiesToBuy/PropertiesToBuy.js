...
import { fetchBuyProperties } from '../../redux/ActionCreators';
...
const RenderPropertyCard = ({property, postFavourite, favourite, deleteFavourite}) => {
    ...
    return (
        <Card onMouseDown={e => e.preventDefault()} className={classes.root}>
            <CardActionArea>
            <Link to={`/properties-to-buy/${property._id}`}>
            <CardMedia
            ...
            <CardContent style={{height: '18em'}}>
                ...
                <span className={classes.title_card}>{property.propertyName}  
                    <span onClick={() => redirectNotUser} className={classes.heart_div} >
                ...
                <b className={classes.price}>{`£${formatter.format(property.salePrice)}`}</b> 
                <p className={classes.mini_description}>{`${property.bedNum} bedroom house for sale`}</p>
                </Typography>
                </Link>
...
function PropertiesToBuy(props) {
    const classes = useStyles()
    ...
    let propertiesList = useSelector(state => state.buyProperties)
    ...
    if (props.properties.isLoading) {
        ...
        return (
            <div>
                <SearchBar criteria={null} page={page} setPage={setPage} />
                <CountProperties category={"sale"} propertiesCount={count}/>
                <div className={classes.container}>
        ...
        if (count === 0){
            return (
                <div>
                    ...
                    <CountProperties category={"sale"} propertiesCount={count} />
                    <div className={classes.container}>
                        <p style={{fontSize: '1.4rem', margin: '1.3em'}}>There are no properties to buy found. Retry with another search</p>
                    </div>
                </div>
            )
        }
...
export default withRouter(PropertiesToBuy)