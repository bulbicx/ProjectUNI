...
function AddProperty({propertyCreate, alert}) {
    const classes = useStyles()

    const properties = useSelector(state => state.properties)
    const {isLoading, errMess} = properties
    ...
    const addFeatureToList = async (e) => {
        e.preventDefault()
        await featuresList.push({
            feat: values.feature.capitalize()
        })
        await featuresListShow.push({
            id: Date.now() + Math.random(),
            value: values.feature.capitalize()
        })
    ...
    const addStationToList = async (e) => {
        e.preventDefault()
        await values.stations.push({
            stationName: values.station.capitalize()
        })
        await stationsShow.push({
            id: Date.now() + Math.random(),
            value: values.station.capitalize()
        })
        setValues(prevValues => ({
            ...prevValues,
            station: ''
    ...
    const handleDeleteFeature = async (uid) => {
        for (var i = 0; i < featuresList.length; i++) {
            if(featuresListShow[i].id == uid) {
                await featuresList.splice(i, 1)
    ...
    const handleDeleteStation = async (uid) => {
        for (var x = 0; x < values.stations.length; x++) {
            if(stationsShow[x].id == uid) {
                await values.stations.splice(x, 1)
    ...
    return (
        <div className={classes.main_container}>
            <Breadcrumb>
                ...
                <BreadcrumbItem active>Add New Property</BreadcrumbItem>
            </Breadcrumb>
            ...
            <div>
                <form className={classes.property_form}>
                    <div className={classes.left_section}>
                        ...
                        <br />
                        <TextField
                            error={values.propertyInfoError ? true : false}
                            className={classes.textField}
                            id="Property info"
                            label="Property info"
                            multiline
                        ...
                        <div className={classes.further_det_station_section}>
                            ...
                            <TextField
                                className={classes.textField}
                                error={values.depositError ? true : false}
                                name="deposit"
                                id="deposit"
                                label="Deposit"
                                variant="outlined"
                            ...
                            <TextField
                                className={classes.textField}
                                error={values.localAuthorityError ? true : false}
                            ...
                            <TextField
                                className={classes.textField}
                                error={values.councilTaxError ? true : false}
                                label="Council tax"
                                type="number"
                                id="council_tax"
                            ...
                            <br />
                            <TextField
                                error={values.squaresError ? true : false}
                                className={classes.textField}
                                label="Squares Ft"
                                type="number"
                                id="Squares_ft"
                            ...
                            <div className={classes.extra_section}>
                                ...
                                <div className={classes.list_add_new}>
                                    ...
                                    />
                                    <button disabled={values.feature.length<1} className={classes.btn_add_feature} onClick={(e) => addFeatureToList(e)}>Add Feature</button>
                                </div>
                            <br />
                                ...
                                featuresListShow.map(feat => {
                                    ...
                                    return (
                                        <div key={ukey} style={{display: 'flex', justifyContent: 'space-between'}}>
                                            <p key={feat.feat} className={classes.feat_text}>{feat.value}</p>
                                            <i onClick={() => handleDeleteFeature(feat.id)} key={feat.id} className={`fas fa-trash-alt ${classes.trash_icon}`}></i>
                                        </div>
                                    )
                                })
                    ...
                    <div className={classes.right_section}>
                        <FormControl className={classes.multiselector} 
                            error={values.featuredError ? true : false} 
                            variant="outlined">
                        ...
                        <FormControl className={classes.multiselector}
                            error={values.bedError ? true : false} 
                            variant="outlined" >
                        ...
                        <FormControl className={classes.multiselector}
                            error={values.bathError ? true : false} 
                            variant="outlined" >
                        ...
                        <FormControl className={classes.multiselector}
                            error={values.petError ? true : false} 
                            variant="outlined" >
                        ...
                        <FormControl className={classes.multiselector}      
                            error={values.schoolError ? true : false} 
                            variant="outlined">
                        ...
                        <FormControl className={classes.multiselector}
                            error={values.crimeRateError ? true : false} 
                            variant="outlined" >
                        ...
                        <FormControl className={classes.multiselector}
                            error={values.trainError ? true : false} 
                            variant="outlined" >
                        ...
                        <FormControl className={classes.multiselector} 
                            error={values.locationAreaError ? true : false} 
                            variant="outlined">
                        ...
                        <div className={classes.list_add_new}>
                            ...
                            />
                            <button disabled={values.station.length<1} className={classes.btn_add_feature} onClick={(e) => addStationToList(e)}>Add Station</button>
                        </div>
                        <br />
                        <div className={classes.feature_list_area}>
                            {
                                stationsShow.map(station => {
                                    ...
                                    return (
                                        <div key={ukey} style={{display: 'flex', justifyContent: 'space-between'}}>
                                            <p key={station.value} className={classes.feat_text}>{station.value}</p>
                                            <i onClick={() => handleDeleteStation(station.id)} key={station.id} className={`fas fa-trash-alt ${classes.trash_icon}`}></i>
                                        </div>
                                    )
                                })
                            }
                        ...
                        <FormControl 
                            className={classes.multiselector}
                            style={{marginRight: '3em'}}
                            error={values.landlordError ? true : false} 
                            variant="outlined" >
                        ...
                        <div className={classes.btn_submit_setion}>
                            ...
                            <button className={classes.btn_submit_form} onClick={(e) => submitHandler(e)}>Add Property</button>
                        </div>
                        
                        {
                            isLoading && 
                            <span style={{margin: '4em 0 0 2em'}}>
                            <img alt="loading" style={{marginRight: '0.5em'}} src="data:image/gif;base64," />
                            Loading...
                            </span>
                        }
                    </div>
